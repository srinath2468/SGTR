...
__global__ void ml_nms_kernel(const int n_boxes, const float nms_overlap_thresh, const float* dev_boxes,
...
at::Tensor ml_nms_cuda(const at::Tensor boxes, float nms_overlap_thresh)
{
  ...
  return std::get<0>(
      order_t
          .index({keep.narrow(/*dim=*/0, /*start=*/0, /*length=*/num_to_keep).to(order_t.device(), keep.scalar_type())})
          .sort(0, false));
}
